...
export class TestTree extends DisposableBase {
	...
	startedLoading() {
		...
		if (this._vscode.workspace.workspaceFolders?.length === 1) {
			const rootItem = this._createInlineRootItem(
				this._vscode.workspace.workspaceFolders[0].uri,
			);
			...
			for (const workspaceFolder of this._vscode.workspace
				.workspaceFolders || []) {
				const rootTreeItem = this._createRootFolderItem(
	...
	private _update() {
		for (const [workspaceFolder, workspaceRootItem] of this._rootItems) {
			...
			for (const model of this._models
				.enabledModels()
				.filter((m) => m.config.workspaceFolder === workspaceFolder)) {
	...
	private _syncSuite(uItem: upstream.TreeItem, vsItem: vscodeTypes.TestItem) {
		...
		vsChildren.forEach((c) => {
			if (c.id.startsWith(this._testGeneration))
				vsChildrenById.set(
					c.id.substring(this._testGeneration.length),
					c,
				);
		});
		...
		for (const [id, uChild] of uChildrenById) {
			...
			if (!vsChild) {
				vsChild = this._testController.createTestItem(
					...
					uChild.title,
					this._vscode.Uri.file(uChild.location.file),
				);
	...
	private _indexTree() {
		...
		for (const item of this._rootItems.values()) visit(item);
	}
	...
	private _createRootFolderItem(folder: string): vscodeTypes.TestItem {
		const folderItem = this._testController.createTestItem(
			this._idWithGeneration(folder),
			path.basename(folder),
			this._vscode.Uri.file(folder),
		);
	...
	): vscodeTypes.TestItem | undefined {
...
export function upstreamTreeItem(
