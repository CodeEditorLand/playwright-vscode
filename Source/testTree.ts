...
export class TestTree extends DisposableBase {
	...
	private _update() {
		for (const workspaceFolder of this._vscode.workspace.workspaceFolders ??
			...
			for (const model of this._models
				...
				.filter(
					(m) =>
						m.config.workspaceFolder === workspaceFolder.uri.fsPath,
				)) {
		...
		for (const itemFsPath of this._rootItems.keys()) {
			if (
				!this._vscode.workspace.workspaceFolders!.find(
					(f) => f.uri.fsPath === itemFsPath,
				)
			)
	...
	private _createRootItemIfNeeded(
		uri: vscodeTypes.Uri,
		...
		if (this._vscode.workspace.workspaceFolders!.length === 1) {
			...
			item = this._testController.createTestItem(
				this._idWithGeneration(uri.fsPath),
