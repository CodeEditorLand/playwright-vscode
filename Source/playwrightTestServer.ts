...
import type { TestServerInterface } from "./upstream/testServerInterface";
...
export class PlaywrightTestServer {
	...
	async listFiles(): Promise<ConfigListFilesReport> {
		...
		if (!testServer)
			throw new Error(
				"Internal error: unable to connect to the test server",
			);
	...
	): Promise<"passed" | "failed" | "interrupted" | "timedout"> {
		const testServer = await this._testServer();
		if (!testServer) return "failed";
		return await this._runGlobalHooksInServer(
			testServer,
			type,
			testListener,
		);
	...
	private async _runGlobalHooksInServer(
		...
		try {
			if (type === "setup") {
				testListener.onStdOut?.(
					"\x1b[2mRunning global setup if any\u2026\x1b[0m\n",
				);
	...
	async runTests(
		...
		runOptions: PlaywrightTestRunOptions,
		reporter: reporterTypes.ReporterV2,
		token: vscodeTypes.CancellationToken,
	): Promise<void> {
		const testServer = await this._testServer();
	...
	async debugTests(
		...
		const configFile = path.relative(
			this._model.config.workspaceFolder,
			this._model.config.configFile,
		);
		...
		const testDirs = this._model
			.enabledProjects()
			.map((project) => project.project.testDir);
		...
		try {
			await this._vscode.debug.startDebugging(undefined, {
				...
				env: {
					...
					PW_TEST_SOURCE_TRANSFORM:
						require.resolve("./debugTransform"),
					PW_TEST_SOURCE_TRANSFORM_SCOPE:
						testDirs.join(pathSeparator),
					PWDEBUG: "console",
				},
			...
			if (
				...
				debugTestServer &&
				!debugTestServer.isClosed()
			)
				await this._runGlobalHooksInServer(
					debugTestServer,
					"teardown",
					reporter,
				);
	...
	async findRelatedTestFiles(
		files: string[],
	): Promise<ConfigFindRelatedTestFilesReport> {
		const testServer = await this._testServer();
		if (!testServer)
			return {
				testFiles: files,
	...
	private async _computeServerUrlPrefix(wsEndpoint: string) {
		...
		if (
			...
			["[::1]", "0.0.0.0"].includes(serverUrl.hostname)
		)
			serverUrl.hostname = "localhost";
		const serverUri = await this._vscode.env.asExternalUri(
			this._vscode.Uri.parse(serverUrl.origin),
		);
	...
	private _testFilesChanged(testFiles: string[]) {
		this._model.testFilesChanged(
			testFiles.map((f) => this._vscode.Uri.file(f).fsPath),
		);
	}
...
function unwrapString(params: { text?: string; buffer?: string }):
	| string
	| Buffer {
	return params.buffer
		? Buffer.from(params.buffer, "base64")
		: params.text || "";
}
