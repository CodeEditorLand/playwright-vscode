...
export class PlaywrightTestCLI {
	...
	async listFiles(): Promise<ConfigListFilesReport> {
		...
		];
		{
			...
			this._log(
				...
				)}> playwright list-files -c ${configFile}`,
			);
		}
	...
	async runTests(
		...
		options: PlaywrightTestRunOptions,
		reporter: reporterTypes.ReporterV2,
		token: vscodeTypes.CancellationToken,
	): Promise<void> {
		const { locations, parametrizedTestTitle } =
			this._narrowDownLocations(items);
		if (!locations) return;
		const args = [];
		this._model
	...
	async _innerSpawn(
...

		{
			...
			this._log(
				...
				)}> playwright test -c ${configFile}${
					printArgs.length ? " " + printArgs.join(" ") : ""
				}${
					relativeLocations.length
		...
		const childProcess = spawn(
			...
			],
			{
				...
				env: {
					...
					PW_TEST_REUSE_CONTEXT: options.reuseContext
						? "1"
						: undefined,
	...
	async debugTests(
		...
		const reporterServer = new ReporterServer(this._vscode);
		const testOptions = await this._options.runHooks.onWillRunTests(
			this._model.config,
			true,
		);
		try {
	...
	async findRelatedTestFiles(
		...
		];
		{
			...
			this._log(
				...
				)}> playwright find-related-test-files -c ${configFile}`,
			);
		}
		try {
			const output = await this._runNode(allArgs, configFolder);
			...
			if (test.uri && test.range) {
				...
				test.parent?.children.forEach((t) => {
					if (
						...
						t.range?.start.line === test.range?.start.line
					)
						++testsAtLocation;
				});
				if (testsAtLocation > 1) parametrizedTestTitle = test.label;
			}
		}
