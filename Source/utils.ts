...
export async function resolveSourceMap(
	file: string,
	fileToSources: Map<string, string[]>,
	...
	if (lastLine?.startsWith("//# sourceMappingURL=")) {
		const sourceMappingFile = path.resolve(
			path.dirname(file),
			lastLine.substring("//# sourceMappingURL=".length),
		);
		try {
			const sourceMapping = await fs.promises.readFile(
				sourceMappingFile,
				"utf-8",
			);
...
async function findNodeViaShell(
	vscode: vscodeTypes.VSCode,
	cwd: string,
	...
	return new Promise<string | undefined>((resolve) => {
		...
		const childProcess = spawn(
			`${vscode.env.shell} -i -c 'echo ${startToken} && which node && echo ${endToken}'`,
			{
				stdio: "pipe",
				shell: true,
				cwd,
...
export async function getPlaywrightInfo(
	vscode: vscodeTypes.VSCode,
	workspaceFolder: string,
	configFilePath: string,
	env: NodeJS.ProcessEnv,
): Promise<{ version: number; cli: string }> {
	const pwtInfo = await runNode(
		vscode,
