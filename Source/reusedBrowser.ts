...
export class ReusedBrowser implements vscodeTypes.Disposable {
	...
	constructor(
		...
		this._disposables.push(
			settingsModel.showBrowser.onChange((value) => {
				if (!value) this.closeAllBrowsers();
			}),
		);
	}
	...
	private async _startBackendIfNeeded(config: TestConfig) {
		...
		const args = [config.cli, "run-server", `--path=/${createGuid()}`];
		...
		this._backend.on("setModeRequested", (params) => {
			if (params.mode === "standby") this._resetNoWait();
		});
		...
		this._backend.on("sourceChanged", async (params) => {
			this._scheduleEdit(async () => {
				...
				if (!params.actions || !params.actions.length) return;
				...
				if (
					...
					params.actions?.length > this._insertedEditActionCount
				) {
				...
				if (params.actions.length) {
					...
					await this._editor.edit(async (editBuilder) => {
						if (!this._editor) return;
						const action =
							params.actions[params.actions.length - 1];
						editBuilder.replace(
							this._editor.selection,
							indentBlock(action, targetIndentation),
						);
					});
	...
	private _scheduleEdit(callback: () => Promise<void>) {
		this._editOperations = this._editOperations
			.then(callback)
	...
	async inspect(models: TestModelCollection) {
		...
		if (
			!selectedModel ||
			!this._checkVersion(selectedModel.config, "selector picker")
		)
			return;
	...
	async record(models: TestModelCollection, recordNew: boolean) {
		const selectedModel = models.selectedModel();
		if (!selectedModel || !this._checkVersion(selectedModel.config)) return;
		if (!this.canRecord()) {
		...
		await this._vscode.window.withProgress(
			...
			async (progress, token) =>
				this._doRecord(progress, selectedModel, recordNew, token),
		);
	}
	...
	private _checkVersion(
		...
		if (config.version < 1.25) {
			this._vscode.window.showWarningMessage(
				this._vscode.l10n.t(
					"Playwright v1.25+ is required for {0} to work, v{1} found",
					message,
					config.version,
				),
			);
		...
		if (
			this._vscode.env.uiKind === this._vscode.UIKind.Web &&
			!process.env.DISPLAY
		) {
			this._vscode.window.showWarningMessage(
				this._vscode.l10n.t(
	...
	private async _doRecord(
		...
		try {
			await this._backend?.setMode({
				mode: "recording",
				testIdAttributeName: model.config.testIdAttributeName,
			});
		} catch (e) {
...
function showExceptionAsUserError(
	vscode: vscodeTypes.VSCode,
	model: TestModel,
	error: Error,
) {
