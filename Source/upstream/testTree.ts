...
export class TestTree {
	...
	constructor(
		...
		projectFilters: Map<string, boolean> | undefined,
		...
		const visitSuite = (
			project: reporterTypes.FullProject,
			parentSuite: reporterTypes.Suite,
			parentGroup: GroupItem,
		) => {
			for (const suite of parentSuite.suites) {
				const title = suite.title || "<anonymous>";

				let group = parentGroup.children.find(
					(item) => item.kind === "group" && item.title === title,
				...
				if (!group) {
					group = {
						...
						id:
							"suite:" +
							parentSuite.titlePath().join("\x1e") +
							"\x1e" +
							title, // account for anonymous suites
						title,
			...
			for (const test of parentSuite.tests) {
				...
				let testCaseItem = parentGroup.children.find(
					(t) => t.kind !== "group" && t.title === title,
				) as TestCaseItem;
				...
				const testItem: TestItem = {
					...
					children: [],
					status,
					duration: test.results.length
						? Math.max(0, test.results[0].duration)
						: 0,
					project,
				};
	...
	filterTree(
		...
		runningTestIds: Set<string> | undefined,
	) {
		...
		const filter = (testCase: TestCaseItem) => {
			const titleWithTags = [
				...testCase.tests[0].titlePath(),
				...testCase.tests[0].tags,
			]
				.join(" ")
				.toLowerCase();
			if (
			...
			testCase.children = (testCase.children as TestItem[]).filter(
				(test) => {
					return (
						!filtersStatuses ||
						runningTestIds?.has(test.test.id) ||
						statusFilters.get(test.status)
					);
				},
			);
	...
	private _fileItem(filePath: string[], isFile: boolean): GroupItem {
		...
		if (existingFileItem) return existingFileItem as GroupItem;

		const parentFileItem = this._fileItem(
	...
	shortenRoot() {
		...
		while (
			shortRoot.children.length === 1 &&
			shortRoot.children[0].kind === "group" &&
