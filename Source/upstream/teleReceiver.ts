...
import type { Annotation, FullProject, Metadata, ReporterV2 } from "./reporter";
...
export type JsonAttachment = Omit<
	reporterTypes.TestResult["attachments"][0],
	"body"
> & { base64?: string };
...
export class TeleReporterReceiver {
	...
	private _onStdIO(
		...
		const result =
			test && resultId ? test._resultsMap.get(resultId) : undefined;
	...
	private _parseAttachments(
		attachments: JsonAttachment[],
	): reporterTypes.TestResult["attachments"] {
		return attachments.map((a) => {
			return {
				...a,
				body:
					a.base64 && (globalThis as any).Buffer
...
export class TeleTestCase implements reporterTypes.TestCase {
	...
	outcome(): "skipped" | "expected" | "unexpected" | "flaky" {
		...
		const failures = results.filter(
			(result) =>
				...
				result.status !== this.expectedStatus,
		);
	...
	ok(): boolean {
		...
		return (
			status === "expected" || status === "flaky" || status === "skipped"
		);
	}
...
class TeleTestStep implements reporterTypes.TestStep {
	...
	constructor(
		payload: JsonTestStepStart,
		parentStep: reporterTypes.TestStep | undefined,
		location: reporterTypes.Location | undefined,
	) {
...
export function serializeRegexPatterns(
...
export function parseRegexPatterns(
