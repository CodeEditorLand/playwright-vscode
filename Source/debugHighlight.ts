...
export class DebugHighlight {
	...
	constructor(vscode: vscodeTypes.VSCode, reusedBrowser: ReusedBrowser) {
		...
		this._disposables = [
			...
			vscode.window.onDidChangeTextEditorSelection((event) => {
				self._highlightLocator(
					event.textEditor.document,
					event.selections[0].start,
				).catch();
			}),
			vscode.window.onDidChangeVisibleTextEditors((event) => {
				pruneHighlightCaches(
					...
					return {
						onDidSendMessage: async (message) => {
							...
							if (
								...
								const catchBlock = message.body.scopes.find(
									(scope: any) =>
										scope.name === "Catch Block",
								);
							...
							if (
								...
								const errorVariable =
									message.body.variables.find(
										(v: any) =>
											v.name === "playwrightError" &&
											v.type &&
											v.type.toLowerCase() === "error",
									);
	...
	private async _highlightLocator(
		...
		const result = await locatorToHighlight(
			this._debugSessions,
			document,
			position,
			token,
...
async function locatorToHighlight(
	debugSessions: Map<string, vscodeTypes.DebugSession>,
	document: vscodeTypes.TextDocument,
	position: vscodeTypes.Position,
	token?: vscodeTypes.CancellationToken,
	...
	if (!debugSessions.size) {
		...
		if (!line.text.match(locatorMethodRegex)) return;

		let locatorExpression = locatorForSourcePosition(
			text,
			{ pages: [], locators: [] },
			fsPath,
			{
		...
		locatorExpression = locatorExpression?.replace(
			/^component\s*\./,
			`page.locator('#root').locator('internal:control=component').`,
		);
		...
		locatorExpression = locatorExpression
			? replaceActionWithLocator(locatorExpression)
			: undefined;
...
async function pausedStackFrames(
	session: vscodeTypes.DebugSession,
	threadId: number | undefined,
): Promise<StackFrame[] | undefined> {
	const { threads } = await session.customRequest("threads").then(
		(result) => result,
		() => ({ threads: [] }),
	);
	...
	for (const thread of threads) {
		...
		try {
			const { stackFrames } = await session
				.customRequest("stackTrace", { threadId: thread.id })
				.then(
					(result) => result,
					() => ({ stackFrames: [] }),
				);
...
async function computeLocatorForHighlight(
	session: vscodeTypes.DebugSession,
	frameId: string,
