...
export function locatorForSourcePosition(
	text: string,
	vars: { pages: string[]; locators: string[] },
	fsPath: string,
	...
	traverse(ast, {
		enter(path) {
			...
			if (
				...
				vars.pages.includes(path.node.callee.object.name) &&
			...
			if (isRangeMatch || isLineMatch) {
				let expression;
				if (pageSelectorNode)
					expression = `${pageSelectorCallee}.locator(${text.substring(
						pageSelectorNode.start!,
						pageSelectorNode.end!,
					)})`;
				else
				...
				if (
					isRangeMatch &&
					(!rangeMatch || expression.length < rangeMatch.length)
				) {
