...
export class TestModel extends DisposableBase {
	...
	constructor(
		...
		if (this._useLegacyCLIDriver)
			this._playwrightTest = new PlaywrightTestCLI(
				this._vscode,
				this,
				collection.embedder,
			);
		...
		this._disposables = [
			...
			this._embedder.settingsModel.embeddedTraceViewer.onChange(() =>
				this.updateTraceViewer(false),
			),
	...
	async _loadModelIfNeeded(configSettings: ConfigSettings | undefined) {
		...
		if (configSettings) {
			...
			if (this.projects().length) this.projects()[0].isEnabled = true;
		}
	}
	...
	testDirs(): string[] {
		return [
			...new Set(
				[...this._projects.values()].map((p) => p.project.testDir),
			),
		];
	}
	...
	enabledProjectsFilter(): string[] {
		const allEnabled = ![...this._projects.values()].some(
			(p) => !p.isEnabled,
		);
	...
	private _updateProjectFiles(
		project: TestProject,
		projectReport: ProjectConfigWithFiles,
	) {
	...
	async handleWorkspaceChange(change: WorkspaceChange) {
		...
		if (created.length || deleted.length) await this._listFiles();
		if (changed.length) {
	...
	async ensureTests(inputFiles: string[]): Promise<void> {
		const enabledFiles = this.enabledFiles();
		const filesToListTests = inputFiles.filter(
			(f) => enabledFiles.has(f) && !this._filesWithListedTests.has(f),
		);
		if (!filesToListTests.length) return;
	...
	private _updateProjects(
		newProjectSuites: reporterTypes.Suite[],
	...
	private _updateFromRunningProject(
		project: TestProject,
		projectSuite: reporterTypes.Suite,
	) {
	...
	canRunGlobalHooks(type: "setup" | "teardown") {
		if (this._useLegacyCLIDriver) return false;
	...
	needsGlobalHooks(type: "setup" | "teardown"): boolean {
		if (type === "setup" && !this._ranGlobalSetup) return true;
		if (type === "teardown" && this._ranGlobalSetup) return true;
		return false;
	}

	async runGlobalHooks(
		type: "setup" | "teardown",
		testListener: reporterTypes.ReporterV2,
		token: vscodeTypes.CancellationToken,
	): Promise<reporterTypes.FullResult["status"]> {
		if (!this.canRunGlobalHooks(type)) return "passed";
		if (type === "setup") {
	...
	async startDevServer() {
		...
		if (result === "passed") this._startedDevServer = true;
	}

	async stopDevServer() {
		if (!this._startedDevServer) return;
		const result = await this._playwrightTest.stopDevServer();
		if (result === "passed") this._startedDevServer = false;
	}
	...
	async runTests(
		...
		if (this.canRunGlobalHooks("setup"))
			globalSetupResult = await this.runGlobalHooks(
				"setup",
				reporter,
				token,
			);
		if (globalSetupResult !== "passed") return;

		const externalOptions = await this._embedder.runHooks.onWillRunTests(
			this.config,
			false,
		);
		const showBrowser =
			this._embedder.settingsModel.showBrowser.get() &&
			!!externalOptions.connectWsEndpoint;

		let trace: "on" | "off" | undefined;
		let video: "on" | "off" | undefined;

		if (this._embedder.settingsModel.showTrace.get()) trace = "on";
	...
	async addToWatch(
		...
		for (const watch of this._watches) {
			if (!watch.include) {
				for (const file of this.enabledFiles()) filesToWatch.add(file);
				continue;
			}
			for (const include of watch.include) {
				if (!include.uri) continue;
		...
		for (const item of items) {
			...
			if (
				...
				for (const file of this.enabledFiles()) {
					if (
						file === treeItem.location.file ||
						file.startsWith(treeItem.location.file)
					)
	...
	updateTraceViewer(userGesture: boolean): TraceViewer | null {
		...
		if (!this._traceViewer && !settingsModel.showTrace.get()) return null;

		if (
			!settingsModel.showTrace.get() ||
		...
		if (
			settingsModel.embeddedTraceViewer.get() &&
			this._traceViewer instanceof EmbeddedTraceViewer
		)
			return this._traceViewer;

		if (
			!settingsModel.embeddedTraceViewer.get() &&
			this._traceViewer instanceof SpawnTraceViewer
		)
			return this._traceViewer;
		...
		if (settingsModel.embeddedTraceViewer.get()) {
			if (
				this._checkVersion(
					1.46,
					this._vscode.l10n.t("embedded trace viewer"),
					userGesture,
				)
			)
				this._traceViewer = new EmbeddedTraceViewer(
					this._vscode,
					this._embedder.context.extensionUri,
					this.config,
					this._playwrightTest as PlaywrightTestServer,
				);
		} else {
			if (
				this._checkVersion(
					1.35,
					this._vscode.l10n.t("this feature"),
					userGesture,
				)
			)
				this._traceViewer = new SpawnTraceViewer(
					this._vscode,
					this._embedder.envProvider,
					this.config,
				);
		}
	...
	private _checkVersion(
		...
		if (this.config.version >= version) return true;
		if (userGesture) {
...
export class TestModelCollection extends DisposableBase {
	...
	setModelEnabled(
		...
		const model = this._models.find(
			(m) => m.config.configFile === configFile,
		);
		if (!model) return;
	...
	private _configSettings(config: TestConfig) {
		const workspaceSettings =
			(this.embedder.context.workspaceState.get(
				workspaceStateKey,
			) as WorkspaceSettings) || {};
		return (workspaceSettings.configs || []).find(
			(c) =>
				c.relativeConfigFile ===
				path.relative(config.workspaceFolder, config.configFile),
		);
	}
	...
	selectedModel(): TestModel | undefined {
		...
		const model = enabledModels.find(
			(m) => m.config.configFile === this._selectedConfigFile,
		);
	...
	private _saveSettings() {
		...
		for (const model of this._models) {
			workspaceSettings.configs!.push({
				relativeConfigFile: path.relative(
