...
export class TestModel {
	...
	constructor(
		...
		this._playwrightTest = this._useLegacyCLIDriver
			? new PlaywrightTestCLI(vscode, this, options)
			: new PlaywrightTestServer(vscode, this, options);
	...
	async runTests(
		...
		const externalOptions = await this._options.runHooks.onWillRunTests(
			this.config,
			false,
		);
		const showBrowser =
			this._options.settingsModel.showBrowser.get() &&
			!!externalOptions.connectWsEndpoint;

		let trace: "on" | "off" | undefined;
		let video: "on" | "off" | undefined;

		if (this._options.settingsModel.showTrace.get()) trace = "on";
...
export class TestModelCollection extends DisposableBase {
	...
	async addModel(model: TestModel) {
		...
		const configSettings = (workspaceSettings.configs || []).find(
			(c) =>
	...
	private async _loadModelIfNeeded(model: TestModel) {
		...
		if (configSettings) {
			...
			if (model.projects().length) model.projects()[0].isEnabled = true;
		}
	}
	...
	clear() {
		this.dispose();
		for (const model of this._models) model.reset();
		this._models = [];
		this._didUpdate.fire();
	}
