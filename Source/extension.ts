...
export class Extension implements RunHooks {
	...
	private async _queueTestRun(
		...
		await this._queueCommand(
			() => this._runTests(include, mode),
			undefined,
		);
	}

	private async _queueWatchRun(
		include: readonly vscodeTypes.TestItem[],
		type: "files" | "items",
	) {
		const batch =
	...
	private async _runTests(
		...
		const requestWithDeps = new this._vscode.TestRunRequest(
			rootItems,
			[],
			undefined,
			mode === "watch",
			...
			onTestEnd: (
				...
				testRun.failed(
					...
					result.errors.map((error) =>
						this._testMessageForTestError(error, testItem),
					),
					result.duration,
				);
			},
	...
	private async _runWatchedTests(
		...
		if (testItems.length) await this._queueWatchRun(testItems, "items");
	}
	...
	private _testMessageForTestError(
      ...
      `);
		...
		const location =
			error.location || parseLocationFromStack(error.stack, testItem);
...
function parseLocationFromStack(
