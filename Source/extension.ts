...
import { type WorkspaceChange, WorkspaceObserver } from "./workspaceObserver";
...
export class Extension implements RunHooks {
	...
	constructor(
		...
		this._traceViewer = new TraceViewer(
			this._vscode,
			this._settingsModel,
			this._envProvider.bind(this),
		);
		...
		for (const configFileUri of configFiles) {
			...
			const model = new TestModel(
				...
				playwrightInfo,
				{
					...
					onStdOut: this._debugHighlight.onStdOut.bind(
						this._debugHighlight,
					),
	...
	private async _runGlobalHooks(type: "setup" | "teardown") {
		...
		try {
			const status =
				(await this._models
					.selectedModel()
					?.runGlobalHooks(type, testListener)) || "failed";
			return status;
		} finally {
			testRun.end();
		}
	...
	private async _runTests(
		...
		try {
			for (const model of this._models.enabledModels()) {
				...
				let globalSetupResult: reporterTypes.FullResult["status"] =
					"passed";
				if (model.canRunGlobalHooks("setup")) {
					const testListener = this._errorReportingListener(
						this._testRun,
						testItemForGlobalErrors,
					);
					globalSetupResult = await model.runGlobalHooks(
						"setup",
						testListener,
					);
				}
	...
	private async _runTest(
		...
		const testListener: reporterTypes.ReporterV2 = {
			...
			onTestBegin: (
				...
				if (testItem && enqueuedSingleTest) this._showTrace(testItem);
				if (isDebug) {
			...
			onTestEnd: (
				...
				if (enqueuedSingleTest) this._showTrace(testItem);
	...
	private async _runWatchedTests(
		...
		if (testItems.length) await this._queueTestRun(testItems, "watch");
	}
	...
	private _testMessageFromText(text: string): vscodeTypes.TestMessage {
		...
		for (let line of text.split("\n")) {
			if (line.startsWith("    at ")) {
				...
				for (const workspaceFolder of this._vscode.workspace
					.workspaceFolders || []) {
					const prefix1 = (
						"    at " +
					...
					if (lowerLine.startsWith(prefix2)) {
						line =
							"    at " +
							line.substring(prefix2.length, line.length - 1);
						break;
					}
				}
			}
		...
		if (logMd.length) markdownString.appendMarkdown(logMd.join("\n"));
	...
	private _showTrace(testItem: vscodeTypes.TestItem) {
		...
		if (testModel) this._traceViewer.open(traceUrl, testModel.config);
	}
