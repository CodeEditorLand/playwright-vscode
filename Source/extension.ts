...
export class Extension implements RunHooks {
	...
	constructor(
		...
		this._activeStepDecorationType =
			this._vscode.window.createTextEditorDecorationType({
				isWholeLine: true,
		...
		this._completedStepDecorationType =
			this._vscode.window.createTextEditorDecorationType({
				isWholeLine: true,
				after: {
					color: { id: "editorCodeLens.foreground" },
		...
		this._reusedBrowser = new ReusedBrowser(
			this._vscode,
			this._settingsModel,
			this._envProvider.bind(this),
		);
		...
		this._testController.resolveHandler = (item) =>
			this._resolveChildren(item);
		this._testController.refreshHandler = () =>
			this._rebuildModels(true).then(() => {});
		const supportsContinuousRun = true;
		this._runProfile = this._testController.createRunProfile(
			"playwright-run",
			this._vscode.TestRunProfileKind.Run,
			this._handleTestRun.bind(this, false),
			true,
			undefined,
			supportsContinuousRun,
		);
		this._debugProfile = this._testController.createRunProfile(
			"playwright-debug",
			this._vscode.TestRunProfileKind.Debug,
			this._handleTestRun.bind(this, true),
			true,
			undefined,
			supportsContinuousRun,
		);
		this._testTree = new TestTree(
			vscode,
			this._models,
			this._testController,
		);
		this._debugHighlight.onErrorInDebugger((e) =>
			this._errorInDebugger(e.error, e.location),
		);
		this._workspaceObserver = new WorkspaceObserver(
			this._vscode,
			(changes) => this._workspaceChanged(changes),
		);
		this._diagnostics = this._vscode.languages.createDiagnosticCollection(
			"pw.testErrors.diagnostic",
		);
	...
	async activate() {
		...
		const messageNoPlaywrightTestsFound = this._vscode.l10n.t(
		...
		this._disposables = [
			...
			vscode.commands.registerCommand(
				"pw.extension.install",
				async () => {
					await installPlaywright(this._vscode);
				},
			),
			vscode.commands.registerCommand(
				"pw.extension.installBrowsers",
				async () => {
					if (!this._models.hasEnabledModels()) {
						vscode.window.showWarningMessage(
							messageNoPlaywrightTestsFound,
						);
						return;
					}
			...
			vscode.commands.registerCommand(
				"pw.extension.command.closeBrowsers",
				() => {
					this._reusedBrowser.closeAllBrowsers();
				},
			),
			vscode.commands.registerCommand(
				"pw.extension.command.recordNew",
				async () => {
					if (!this._models.hasEnabledModels()) {
						vscode.window.showWarningMessage(
							messageNoPlaywrightTestsFound,
			...
			vscode.commands.registerCommand(
				"pw.extension.command.recordAtCursor",
				async () => {
					if (!this._models.hasEnabledModels()) {
						vscode.window.showWarningMessage(
							messageNoPlaywrightTestsFound,
			...
			vscode.commands.registerCommand(
				"pw.extension.command.toggleModels",
				async () => {
					this._settingsView.toggleModels();
				},
			),
			vscode.commands.registerCommand(
				"pw.extension.command.runGlobalSetup",
				async () => {
			...
			vscode.commands.registerCommand(
				"pw.extension.command.runGlobalTeardown",
				async () => {
			...
			vscode.commands.registerCommand(
				"pw.extension.command.startDevServer",
				async () => {
			...
			vscode.commands.registerCommand(
				"pw.extension.command.stopDevServer",
				async () => {
			...
			vscode.commands.registerCommand(
				"pw.extension.command.clearCache",
				async () => {
			...
			this._treeItemObserver.onTreeItemSelected((item) =>
				this._treeItemSelected(item),
			),
			this._settingsView,
			this._testController,
			this._runProfile,
	...
	private async _rebuildModels(
		...
		const configFiles = await this._vscode.workspace.findFiles(
			"**/*playwright*.config.{ts,js,mts,mjs}",
			"**/node_modules/**",
		);
		...
		for (const configFileUri of configFiles) {
			...
			const workspaceFolder =
				this._vscode.workspace.getWorkspaceFolder(configFileUri)!;
			const workspaceFolderPath = workspaceFolder.uri.fsPath;
			if (
				configFilePath.includes("test-results") &&
				!workspaceFolderPath.includes("test-results")
			)
				continue;

			let playwrightInfo = null;
			try {
				playwrightInfo = await getPlaywrightInfo(
					this._vscode,
					workspaceFolderPath,
					configFilePath,
					this._envProvider(),
				...
				if (userGesture) {
					this._vscode.window.showWarningMessage(
						error instanceof NodeJSNotFoundError
							? error.message
							: this._vscode.l10n.t(
									"Please install Playwright Test via running `npm i --save-dev @playwright/test`",
								),
					);
				}
			...
			if (playwrightInfo.version < minimumPlaywrightVersion) {
				if (userGesture) {
					this._vscode.window.showWarningMessage(
						this._vscode.l10n.t(
							"Playwright Test v{0} or newer is required",
							minimumPlaywrightVersion,
						),
					);
				}
				continue;
			...
			await this._models.createModel(
				workspaceFolderPath,
	...
	private _envProvider(): NodeJS.ProcessEnv {
		const env = this._vscode.workspace
	...
	private async _queueTestRun(
		request: vscodeTypes.TestRunRequest,
		mode: "run" | "debug",
	) {
		await this._queueCommand(
			() => this._runTests(request, mode),
			undefined,
		);
	}

	private async _queueWatchRun(
		request: vscodeTypes.TestRunRequest,
		type: "files" | "items",
	) {
		const batch =
			type === "files" ? this._watchFilesBatch : this._watchItemsBatch;
		...
		if (type === "files") this._watchFilesBatch = [...include];
		else this._watchItemsBatch = [...include];

		await this._queueCommand(() => {
			const items =
				type === "files"
					? this._watchFilesBatch
					: this._watchItemsBatch;
			if (typeof items === "undefined")
				throw new Error(
					`_watchRunBatches['${type}'] is undefined, expected array`,
				);

			if (type === "files") this._watchFilesBatch = undefined;
			else this._watchItemsBatch = undefined;

			return this._runTests(request, "watch");
		}, undefined);
	}

	private async _queueGlobalHooks(
		type: "setup" | "teardown",
	): Promise<reporterTypes.FullResult["status"]> {
		return await this._queueCommand(
			() => this._runGlobalHooks(type),
			"failed",
		);
	}
	...
	private async _runGlobalHooks(type: "setup" | "teardown") {
		...
		try {
			const status =
				(await this._models
					.selectedModel()
					?.runGlobalHooks(type, testListener, testRun.token)) ||
				"failed";
			return status;
		} finally {
			testRun.end();
	...
	private async _runTests(
		...
		if (!testItemForGlobalErrors) {
			for (const rootItem of rootItems) {
				if (!rootItem.children.size) continue;
				rootItem.children.forEach((c) => {
					if (!testItemForGlobalErrors) testItemForGlobalErrors = c;
				});
				if (testItemForGlobalErrors) break;
			}
		}
		...
		try {
			for (const model of this._models.enabledModels()) {
				...
				if (!result.testIds && !result.locations) continue;
				if (!model.enabledProjects().length) continue;
				await this._runTest(
					this._testRun,
					request,
	...
	private async _resolveChildren(
		fileItem: vscodeTypes.TestItem | undefined,
	): Promise<void> {
		if (!fileItem) return;
	...
	private async _runTest(
		...
		const testListener: reporterTypes.ReporterV2 = {
			...
			onBegin: (rootSuite: reporterTypes.Suite) => {
				...
				for (const test of rootSuite.allTests()) {
					...
					if (testItem) testRun.enqueued(testItem);
				}
			},
			...
			onTestEnd: (
				...
				const trace =
					result.attachments.find((a) => a.name === "trace")?.path ||
					"";
				...
				if (result.status === test.expectedStatus) {
					if (!testFailures.has(testItem)) {
						...
						else testRun.passed(testItem, result.duration);
					}
					return;
				}
				...
				testRun.failed(
					...
					result.errors.map((error) =>
						this._testMessageForTestError(error),
					),
					result.duration,
				);
			},
			...
			onStepBegin: (
				test: reporterTypes.TestCase,
				result: reporterTypes.TestResult,
				testStep: reporterTypes.TestStep,
			) => {
				if (!testStep.location) return;
				let step = this._activeSteps.get(testStep);
				if (!step) {
					step = {
						location: new this._vscode.Location(
							this._vscode.Uri.file(testStep.location.file),
							new this._vscode.Position(
								testStep.location.line - 1,
								testStep.location?.column - 1,
							),
						),
						activeCount: 0,
						duration: 0,
			...
			onStepEnd: (
				...
				if (step.activeCount === 0) this._activeSteps.delete(testStep);
				this._executionLinesChanged();
			},
		};
		...
		if (mode === "debug") {
			...
			await this._models
				.selectedModel()
				?.updateTraceViewer(mode === "run")
				?.willRunTests();
			await model.runTests(request, testListener, testRun.token);
		}
	}

	private _errorReportingListener(
		testRun: vscodeTypes.TestRun,
		testItemForGlobalErrors?: vscodeTypes.TestItem,
	) {
		const testListener: reporterTypes.ReporterV2 = {
			onStdOut: (data) => {
			...
			onError: (error: reporterTypes.TestError) => {
				...
				if (testItemForGlobalErrors) {
					...
					testRun.appendOutput(
						...
						new this._vscode.Location(
							this._vscode.Uri.file(error.location.file),
							new this._vscode.Position(
	...
	private async _runWatchedTests(
		files: string[],
		...
		if (files.length) {
			const fileItems = files
				.map((f) => this._testTree.testItemForFile(f))
				.filter(Boolean) as vscodeTypes.TestItem[];
			await this._queueWatchRun(
				new this._vscode.TestRunRequest(fileItems),
				"files",
			);
		}
		if (testItems.length)
			await this._queueWatchRun(
				new this._vscode.TestRunRequest(testItems),
				"items",
			);
	}

	private async _updateVisibleEditorItems() {
		const files = this._vscode.window.visibleTextEditors.map(
	...
	private _updateDiagnostics() {
		...
		const diagnosticsByFile = new Map<
			string,
			Map<string, vscodeTypes.Diagnostic>
		>();
		...
		const addError = (error: reporterTypes.TestError) => {
			...
			if (!diagnostics.has(key)) {
				diagnostics.set(key, {
					...
					source: "playwright",
					range: new this._vscode.Range(
						Math.max(error.location!.line - 1, 0),
						Math.max(error.location!.column - 1, 0),
						error.location!.line,
						0,
					),
					message: this._abbreviateStack(
						stripBabelFrame(stripAnsi(error.message!)),
					),
				});
			}
		};
		...
		for (const model of this._models.enabledModels()) {
			for (const error of model.errors().values()) addError(error);
		}
		for (const [file, diagnostics] of diagnosticsByFile)
			this._diagnostics.set(this._vscode.Uri.file(file), [
				...diagnostics.values(),
			]);
	}

	private _errorInDebugger(
		errorStack: string,
		...
		const position = new this._vscode.Position(
			location.line - 1,
			location.column - 1,
		);
		testMessage.location = new this._vscode.Location(
			this._vscode.Uri.file(location.file),
			position,
		);
	...
	private _abbreviateStack(text: string): string {
		const result: string[] = [];
		const prefixes = (this._vscode.workspace.workspaceFolders || []).map(
			(f) => f.uri.fsPath.toLowerCase() + path.sep,
		);
	...
	private _testMessageForTestError(
		error: reporterTypes.TestError,
	): vscodeTypes.TestMessage {
      ...
      `);
		...
		const stackTrace = error.stack
			? parseStack(this._vscode, error.stack)
			: [];
		const location = error.location
			? parseLocation(this._vscode, error.location)
			: topStackFrame(this._vscode, stackTrace);
		if (stackTrace.length) testMessage.stackTrace = stackTrace;
		if (location) testMessage.location = location;
		return testMessage;
	}
	...
	private _queueCommand<T>(
		callback: () => Promise<T>,
		defaultValue: T,
	): Promise<T> {
		const result = this._commandQueue.then(callback).catch((e) => {
			console.error(e);
			return defaultValue;
		});
...
function topStackFrame(
	vscode: vscodeTypes.VSCode,
	stackTrace: vscodeTypes.TestMessageStackFrame[],
): vscodeTypes.Location | undefined {
	return stackTrace.length
		? new vscode.Location(stackTrace[0].uri!, stackTrace[0].position!)
		: undefined;
}
...
function parseStack(
	vscode: vscodeTypes.VSCode,
	stack: string,
): vscodeTypes.TestMessageStackFrame[] {
	const lines = stack?.split("\n") || [];
	const result: vscodeTypes.TestMessageStackFrame[] = [];
	for (const line of lines) {
		const frame = stackUtils.parseLine(line);
		if (!frame || !frame.file || !frame.line || !frame.column) continue;
		result.push(
...
class TreeItemObserver implements vscodeTypes.Disposable {
	...
	private async _poll() {
		clearTimeout(this._timeout);
		const result = (await this._vscode.commands.executeCommand(
			"testing.getExplorerSelection",
		)) as { include: vscodeTypes.TreeItem[] };
