...
const upstreamFiles = new Map([
	...
	[
		"src/upstream/testServerConnection.ts",
...
(async () => {
	...
	for (const packageName of packageNames) {
		...
		console.log(
			execSync(
				`node ./utils/pack_package ${packageName} ` +
					path.join(__dirname, `../out/${packageName}.tgz`),
	...
	for (const [to, from] of upstreamFiles) {
		...
		if (fromSuffix)
			await fs.promises.writeFile(
				toPath,
				toPrefix + downstreamBoundary + fromSuffix,
				"utf8",
			);
		else await fs.promises.writeFile(toPath, fromPrefix, "utf8");
	}
})();
